...
// Using the correct TTS model name as per documentation
...
export async function generateDialogueAudio(scenario: HistoricalScenario): Promise<AudioBuffer> {
  ...
  const performGeneration = async (useTranslation: boolean): Promise<AudioBuffer> => {
      let dialogueText = `TTS the following conversation between ${safeNames[0]} and ${safeNames[1]}:\n`;
  ...
  try {
    ...
    console.warn("Primary TTS generation failed, attempting fallback to English translation.", error);
    try {
